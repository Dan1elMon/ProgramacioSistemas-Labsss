...
void mostrarPalabra(const char* palabra, const vector<char>& letrasAdivinadas, bool mostrarGuiones) {
...
int main() {
    ...
    while (vidas > 0 && !palabraAdivinada(palabra, letrasAdivinadas)) {
        mostrarPalabra(palabra, letrasAdivinadas, true); //Mostrar palabra con guiones
        ...
        cout << "Correcto! La letra '" << guess << "' está en la palabra." << endl;
        } else {
        vidas--;
