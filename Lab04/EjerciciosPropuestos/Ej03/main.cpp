...
bool verificarPuntosSilla(const vector<vector<double>>& matrix, int fila, int columna) {
...
void buscarPuntoSilla(const vector<vector<double>>& matrix) {
    ...
    for (int fila = 0; fila < matrix.size(); fila++) {
        ...
        if (verificarPuntosSilla(matrix, fila, columna)) {
            cout << "Punto de silla encontrado en la posicion (" << fila + 1 << ", " << columna + 1 << ") con valor: " << matrix[fila][columna] << endl;
            return;
        }
        }
    }
